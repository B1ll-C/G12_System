<?php
	...
	if (mysqli_num_rows($reserve_result) > 0) {
		while ($reserve_row = mysqli_fetch_array($reserve_result)) {
			$rrr 		= $reserve_row[0];
			$lr 		= "SELECT Dtime FROM $rrr WHERE Dtime LIKE '%$d2%' ";
...
?>
						<li><button style="background-color: #C67D01;" value="<?php echo $reserve_row[0]?>" name="reserve_day3"><?php echo $acc_result;?></button></li>
